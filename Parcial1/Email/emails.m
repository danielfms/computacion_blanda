...
%Xtesting Normalizacion de los datos
featurest = columns(Xtesting);
St = max(Xtesting)-min(Xtesting);
Ut = mean(Xtesting); %promedio

#for i=1:featurest
#    Xtesting(:,i) = (Xtesting( : , i ) - Ut(i)) / St(i);
#end
...
for i = 1:MAX_ITR
    ...
    H= 1 ./ ( 1 + exp( -Z ));
    W = W - ( alpha / m )*( X' * ( H - Y ));
    J = [J  ( - 1 / m ) * ( ( Y' * log(H+0.0000000001) ) + ( ( 1 - Y )' * log( 1 - H ) ) )];
